/*
  # Migration Afrilyft - Système de ride-sharing

  1. Nouvelles Tables
    - `profiles` - Profils utilisateurs (customers et drivers)
    - `driver_locations` - Positions des chauffeurs en temps réel
    - `rides` - Trajets demandés par les clients
    - `ride_requests` - Demandes envoyées aux chauffeurs

  2. Fonctions
    - `calculate_distance` - Calcul de distance avec formule Haversine
    - `find_nearby_drivers` - Recherche de chauffeurs à proximité
    - `create_ride_and_notify_drivers` - Création de trajet et notification
...
CREATE TABLE IF NOT EXISTS rides (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL, -- Référence au client qui demande le trajet
  driver_id uuid, -- Référence au chauffeur assigné
...
DO $$
BEGIN
  -- Contraintes pour driver_locations
  IF NOT EXISTS (
  ...
  -- Contraintes pour rides
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'rides_user_id_fkey'
  ) THEN
    ALTER TABLE rides 
