/*
  # Complete AfriLyft Database Schema Migration

  1. New Tables
    - `profiles` - User profiles with role-based access
    - `ride_requests` - Driver ride request management
    - `notifications` - User notification system

  2. Enhanced Tables
    - Add missing columns to `driver_locations` (heading, speed, is_available)
    - Add missing columns to `rides` (fare_amount, distance_km, etc.)

  3. Functions
    - `calculate_distance` - Haversine distance calculation
    - `find_nearby_drivers` - Find available drivers within radius
    - `create_ride_and_notify_drivers` - Create ride and notify nearby drivers
    - `cleanup_expired_requests` - Clean up expired ride requests

  4. Security
    - Enable RLS on all tables
    - Add comprehensive policies for role-based access
    - Secure data access between customers and drivers

  5. Performance
    - Add indexes for geolocation queries
    - Add indexes for status and user-based queries
...
-- Supprimer la fonction existante si elle existe avec une signature différente
DROP FUNCTION IF EXISTS find_nearby_drivers(double precision, double precision, double precision, integer);
...
DROP FUNCTION IF EXISTS create_ride_and_notify_drivers(uuid, double precision, double precision, text, double precision, double precision, text, text, text, timestamptz);
...
-- Fonction trigger pour nettoyer automatiquement les demandes expirées
...
-- Trigger pour nettoyer les demandes expirées après chaque insertion
...
CREATE POLICY "Users can read other profiles for rides"
  ...
  USING (
    ...
    EXISTS (
      SELECT 1 FROM rides 
      WHERE (customer_id = auth.uid() AND driver_id = profiles.id) 
         OR (driver_id = auth.uid() AND customer_id = profiles.id)
    )
  );
