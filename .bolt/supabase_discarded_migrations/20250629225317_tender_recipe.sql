/*
  # Migration pour système de ride-sharing

  1. Nouvelles Tables
    - `profiles` - Profils utilisateurs avec rôles (customer/driver)
    - `ride_requests` - Demandes de trajets envoyées aux chauffeurs
    
  2. Modifications Tables Existantes
    - Ajout de colonnes manquantes à `driver_locations`
    - Ajout de colonnes manquantes à `rides`
    
  3. Fonctions
    - `calculate_distance()` - Calcul distance Haversine
    - `find_nearby_drivers()` - Recherche chauffeurs à proximité
    - `create_ride_and_notify_drivers()` - Création trajet et notification
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE rides ADD COLUMN fare_amount decimal(10,2);
  END IF;
...
BEGIN
  ...
  SELECT 
    dl.driver_id,
    COALESCE(p.full_name, u.email) as driver_name,
  ...
  LEFT JOIN auth.users u ON u.id = dl.driver_id
  WHERE 
...
CREATE POLICY "Customers can read ride requests for their rides"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM rides 
      WHERE id = ride_requests.ride_id AND user_id = auth.uid()
    )
  );